...
单二进制 ETL/CDC 编排引擎，用 pipeline 完成数据同步、加工和运行治理。

[English README](./README.md)

OpenETL-Go 运行 `Source -> Transform -> Sink` 管道。简单场景可以是一条文件或数据库同步任务；复杂场景可以升级为 DAG 编排、并行分片和 master-worker 分布式执行，用多源、多分支、join、window 和多 sink 表达完整链路。
...
| Transforms | `normalize_envelope`、`filter`、`validate`、`type_convert`、`rename`、`drop_field`、`add_field`、`deduplicate`、`lookup`、`enricher`、`join`、`window`、`router`、`fanout`、`tap`、`rate_limiter`、`lua`、`javascript`、`typescript`、WASM 插件 |
