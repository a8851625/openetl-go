...
podman exec "$MYSQL_CONTAINER" mysql -uroot -proot123456 dzh3136_go -e "INSERT INTO customers (id, name, email, phone, status, amount) VALUES (9201, 'Snapshot CDC Alice', 'snapshot-cdc@example.com', '13900009201', 'active', 321.00); UPDATE customers SET amount=654.00 WHERE id=9201;"

echo "==> Verify CDC copied"
i=0
while [ "$i" -lt 60 ]; do
  copied="$(podman exec "$MYSQL_CONTAINER" mysql -N -usync_user -psync_password_123 -e "SELECT COUNT(*) FROM dzh3136_target.customers WHERE id=9201 AND amount=654.00;" 2>/dev/null | tr -d '[:space:]')"
