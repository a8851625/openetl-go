...
TESTS=(
  ...
  "e2e-wasm-plugin.sh|Real TypeScript WASM install/output/DLQ/replay/restart"
  ...
  "e2e-clickhouse-replay-ordering.sh|ClickHouse source-order replay (snapshot+CDC native + Kafka HTTP)"
  ...
  "e2e-kafka-canal-identity.sh|Kafka Canal identity + frozen/legacy DLQ replay gate"
  ...
  "e2e-lifecycle-desired-state.sh|Durable stop/restart zero-I/O + checkpoint resume"
