...
target_count() {
  docker exec "$MYSQL_CONTAINER" mysql -N -usync_user -psync_password_123 -e "SELECT COUNT(*) FROM dzh3136_target.customers WHERE id IN (9301,9302);" 2>/dev/null | tr -d '[:space:]'
}
...
docker exec "$MYSQL_CONTAINER" mysql -uroot -proot123456 -e "CREATE DATABASE IF NOT EXISTS dzh3136_target; CREATE TABLE IF NOT EXISTS dzh3136_target.customers LIKE dzh3136_go.customers; DELETE FROM dzh3136_go.customers WHERE id IN (9301,9302); DELETE FROM dzh3136_target.customers WHERE id IN (9301,9302); GRANT ALL PRIVILEGES ON dzh3136_target.* TO 'sync_user'@'%'; FLUSH PRIVILEGES;"
...
docker exec "$MYSQL_CONTAINER" mysql -uroot -proot123456 dzh3136_go -e "INSERT INTO customers (id, name, email, phone, status, amount) VALUES (9301, 'CDC Crash One', 'cdc-crash-1@example.com', '13900009301', 'active', 101.00);"
...
docker exec "$MYSQL_CONTAINER" mysql -uroot -proot123456 dzh3136_go -e "INSERT INTO customers (id, name, email, phone, status, amount) VALUES (9302, 'CDC Crash Two', 'cdc-crash-2@example.com', '13900009302', 'active', 202.00); UPDATE customers SET amount=303.00 WHERE id=9302;"

i=0
while [ "$i" -lt 60 ]; do
  count="$(target_count)"
  amount="$(docker exec "$MYSQL_CONTAINER" mysql -N -usync_user -psync_password_123 -e "SELECT COUNT(*) FROM dzh3136_target.customers WHERE id=9302 AND amount=303.00;" 2>/dev/null | tr -d '[:space:]')"
