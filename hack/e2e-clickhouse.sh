...
if ! podman image exists docker.io/clickhouse/clickhouse-server:24.3-alpine; then
  echo "ClickHouse image is not available locally. Pull it first: podman pull docker.io/clickhouse/clickhouse-server:24.3-alpine" >&2
  exit 2
fi
...
podman-compose -f docker-compose.dev.yml up -d mysql-source clickhouse
...
podman exec "$CH_CONTAINER" clickhouse-client --password dzh123456 --multiquery < testdata/clickhouse/init/01-init.sql
podman exec "$CH_CONTAINER" clickhouse-client --password dzh123456 --query "TRUNCATE TABLE dzh3136_go.customers"

echo "==> Reset test row in MySQL"
podman exec "$MYSQL_CONTAINER" mysql -uroot -proot123456 dzh3136_go -e "DELETE FROM customers WHERE id=9101;"
...
podman exec "$MYSQL_CONTAINER" mysql -uroot -proot123456 dzh3136_go -e "INSERT INTO customers (id, name, email, phone, status, amount) VALUES (9101, 'CH CDC Alice', 'ch-cdc@example.com', '13900009101', 'active', 111.11); UPDATE customers SET amount=222.22 WHERE id=9101;"

echo "==> Verify ClickHouse row"
i=0
while [ "$i" -lt 90 ]; do
  copied="$(podman exec "$CH_CONTAINER" clickhouse-client --password dzh123456 --query "SELECT count() FROM dzh3136_go.customers FINAL WHERE id=9101 AND amount=222.22" 2>/dev/null | tr -d '[:space:]')"
  if [ "$copied" = "1" ]; then
    break
  fi
  i=$((i + 1))
