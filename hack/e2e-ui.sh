...
preflight_failed="$(curl -fsS -X POST "${BASE_URL}/api/v2/specs/validate" \
  ...
  -d '{"spec":{"name":"ui-wizard-file","source":{"type":"file","config":{"path":"/app/testdata/files/customers.jsonl","format":"json"}},"transforms":[{"type":"identity","config":{}}],"sink":{"type":"maxcompute","config":{"endpoint":"https://service.cn.maxcompute.aliyun.com/api","project":"demo_project","table":"wizard_output","access_key_id":"replace-me","access_key_secret":"replace-me","columns":{"id":"BIGINT","name":"STRING","dt":"STRING"},"partition_fields":["dt"]}},"batch_size":100,"checkpoint_interval_sec":1,"backpressure_buffer":100,"retry":{"max_attempts":3,"initial_interval_ms":100,"max_interval_ms":1000},"dlq":{"enable":true}}}' | grep -q 'maxcompute-writer' && echo true || echo false)"
