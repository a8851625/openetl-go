...
#
#  前提:  podman 已安装，镜像 openetl-go-etl:ui-e2e 已构建
#
#  本脚本会:
...
#
#  清理:  podman rm -f etl-mysql-src etl-pg-dst etl-demo
# ============================================================
set -euo pipefail
...
# ── 0. 清理旧容器 ──
info "清理旧容器..."
podman rm -f $MYSQL_C $PG_C $APP_C 2>/dev/null || true
rm -rf "$DATA_DIR"
mkdir -p "$DATA_DIR"/{output,checkpoint,dlq}

# ── 1. 启动 MySQL ──
info "启动 MySQL 8 源库 (端口 3316)..."
podman run -d --name $MYSQL_C \
...
for i in $(seq 1 60); do
  podman exec $MYSQL_C mysql -uetl -petl123 -e "SELECT 1" shop >/dev/null 2>&1 && break
  sleep 2
done
ok "MySQL 就绪"
...
for i in $(seq 1 40); do
  podman exec $PG_C psql -U etl -d analytics -c "SELECT 1" >/dev/null 2>&1 && break
  sleep 2
done
ok "PostgreSQL 就绪"
...
# ── 3. 建表 + 插入数据 ──
info "创建 MySQL 表结构..."
podman exec $MYSQL_C mysql -uetl -petl123 shop -e "
CREATE TABLE users (
...
podman exec $PG_C psql -U etl -d analytics -c "
...
echo "  清理环境:    podman rm -f $MYSQL_C $PG_C $APP_C"
echo "═══════════════════════════════════════════════"
