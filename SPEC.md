...
| P5-8 | HTTP source advances `committedPage` before records reach sink (`http.go:225-228`) → crash mid-batch skips the rest of page N. | Advance only when buffer fully drained (as the single-record path already does). | 0.5d | new |
| P5-9 | DLQ write-failure path never trips the circuit breaker (`pipeline.go:913-922`, `executor.go:599-611`) → persistent DLQ outage loses every record, no cooldown. | `circuitBreaker.RecordFailure(ctx, dlqErr)` after the alert. | 0.25d | new |
