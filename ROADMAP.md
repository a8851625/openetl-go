...
| **A11-redo (a)** ✅ | **Implemented 2026-06-22.** `etl.role` (standalone\|master\|worker) + `ETL_ROLE` env; master-role parallel pipelines create `task_assignments` (with shard metadata) and wait via `ShardDispatcher.WaitShard`; worker-role processes poll + execute via `worker.ExecuteShard` (`pipeline.BuildShardRunner`). `pipeline.NewDistributedPipeline`, `master.TaskDispatcher` implements `ShardDispatcher`. | 3 integration tests PASS: `TestDistributedDispatchMySQLReal` (4 shards split 2/2, no overlap, real HTTP workers) + `TestDistributedReassignOnWorkerLossMySQL` (crash reassignment). `e2e-distributed.sh` PASS. 3-OS-process automated E2E pending. | done (~1.5d) | ST-2 |
