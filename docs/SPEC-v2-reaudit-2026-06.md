...
| P4-14 | Window transform has no watermark → late/out-of-order records double-count; sliding/session must not be advertised in the pipeline spec | §1.4 | 🆕 open (TF-8) |
...
- **TF-8 (P1)** — Window has no watermark; sliding/session must not be advertised — `transform/window.go:181-202` should keep the production spec to tumbling-only. Out-of-order CDC replays silently double-count aggregates. **Fix:** track `maxEventTimeSeen`, allow `allowed_lateness`, drop/DLQ older records; reject unsupported `sliding` / `session` config.
