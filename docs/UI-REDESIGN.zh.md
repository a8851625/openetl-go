...
- [x] 可从空环境不编辑 YAML 完成 Connection → Source → Sink → dry-run → preflight → 创建启动（保留 FirstTaskWizard，入口 `#/pipelines/new`）。
...
- [x] Playwright 全量 e2e：`hack/e2e-ui.sh` **107 passed, 0 failed**（脚本已对齐 Overview/问题优先总览/隐藏空态 DLQ 危险按钮/高级 DAG 文案）。

### 实施落点（2026-07-20）

| 能力 | 文件 |
|---|---|
| Health view-model | `web/src/lib/pipeline-health.ts` |
| Hash 路由 | `web/src/lib/routing.ts`, `web/src/main.tsx` |
| 分组导航 / AppShell | `web/src/components/layout/app-shell.tsx` |
| 视觉 tokens | `web/src/styles.css`, `web/tailwind.config.ts` |
| 总览问题优先 | `web/src/pages/DashboardPage.tsx` |
| 问题中心 | `web/src/pages/IssuesPage.tsx` |
| 管道详情 tabs | `web/src/pages/pipelines/PipelineDetailPage.tsx` |
| 列表路径/健康 | `web/src/pages/pipelines/PipelinesPage.tsx` |
| Connector catalog | `web/src/pages/ConnectorsPage.tsx` |
| DLQ 聚合/空态 | `web/src/pages/DLQPage.tsx` |
