...
Kafka ODS JSON -> MaxCompute 分区表的 experimental connector 合约。当前构建会注册 sink，并校验配置、schema 和分区字段；真实 MaxCompute 批写仍需后续接入 SDK client 和集成测试环境后启用。
...
使用该 sink 前应通过 `project` / `type_convert` 让 record schema 匹配声明的 MaxCompute `columns`，并包含所有动态 `partition_fields` 字段。在真实 writer 完成前，`Open()` 会返回明确的 experimental 错误，preflight/UI 应把它视为设计期能力。
