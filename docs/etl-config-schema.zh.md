...
| `allow_unsafe` | 否 | 显式允许 CDC/流式 source 写入非幂等 `file_sink`/`s3`。默认 `false`；仅在已经记录并测试重复吸收与 replay 边界时启用。 |
...
linear runner 与 DAG executor 现在都会保存 checkpoint envelope：sink 写入成功后，把 source position、最新 state snapshot version 与 sink acknowledgement metadata 绑定保存；source 重新打开前会自动把 envelope 解回原始 source position。如果状态快照或 sink acknowledgement metadata 采集失败，source checkpoint 不会前进。
