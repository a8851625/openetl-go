...
> 状态：`complete`（T2.1–T2.8 全部完成；16 项验收标准逐条 passed，见 tasks.md T2.8 验收矩阵） | 依赖：IT-1（已完成） | 归属 roadmap 条目：RA-1、RA-2、RA-3、GAP-7.1/.2/.3
...
| 类型 | 内容 | 状态 |
| --- | --- | --- |
| 迭代依赖 | IT-1（乱序 replay 矩阵与跨路径 e2e 需要其框架） | `complete` |
| 迭代依赖 | IT-1/T1.9（GAP-1/GAP-3 的 PG metadata 已在真实实例验证） | `complete` |
| 外部输入 | 无 | — |
| 决策记录 | 用户已授权按现有 roadmap 持续交付全部可执行迭代；RA-1 已在 IT-1 后作为 IT-2/T2.4 交付 | `resolved`（2026-09-05） |
