...
| `P4` | `delivered` | 首次任务向导、状态语义、deep link、故障自助；`hack/e2e-ui.sh` 108 passed/0 failed（2026-07-25） |
...
当前基线：`hack/e2e-ui.sh` 为 108 passed/0 failed（2026-07-25 收口）。修复覆盖：分步 wizard 导航与 transform stage dry-run/reorder、behavior-scope hint、runtime recommendation YAML 同步、DLQ record filter；e2e 不再依赖已移除的 Failure demo / Repair 专用控件。
