...
export function ErrorBox({ message, className }: { message: string; className?: string }) {
  return (
    <div
      role="alert"
      className={cn(
        'rounded-lg border border-rose-200 bg-rose-50 px-3 py-2.5 text-sm text-rose-700 break-words whitespace-pre-wrap dark:border-rose-900 dark:bg-rose-950/40 dark:text-rose-300',
        className,
      )}
    >
      {message}
