...
func connectorEvidence(kind, typ string) string {
	evidence := map[string]string{
		...
		"source:mysql_snapshot_cdc": "hack/e2e-snapshot-cdc.sh, hack/e2e-snapshot-cdc-clickhouse.sh, and hack/e2e-snapshot-cdc-crash.sh cover integrated snapshot+CDC",
		...
		"sink:mysql":                "hack/e2e.sh, hack/e2e-cdc-mysql.sh, and hack/e2e-debezium-mysql.sh cover MySQL sink upsert/replay",
		...
		"sink:clickhouse":           "hack/e2e-clickhouse.sh, hack/e2e-clickhouse-autocreate.sh, and hack/e2e-snapshot-cdc-clickhouse.sh cover ClickHouse sink",
