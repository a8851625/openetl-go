...
func (r *pgCDCReader) parseInsertMsg(data []byte, lsn string) []byte {
	...
	r.sendRecord(core.Record{
		...
		Metadata: core.Metadata{
			...
			SourceType:        core.SourceTypePostgresCDC,
			...
			PrimaryKeyColumns: append([]string(nil), r.catalog.primaryKeys(tableName)...),
		...
		for rows.Next() {
			...
			rec := core.Record{
				...
				Metadata: core.Metadata{
					...
					Table:             t,
					Timestamp:         time.Now(),
					PrimaryKeyColumns: append([]string(nil), r.catalog.primaryKeys(t)...),
				},
			}
			select {
			case r.records <- rec:
